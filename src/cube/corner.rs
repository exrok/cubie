...
impl std::convert::From<u8> for Corner {
    fn from(a: u8) -> Corner {
        unsafe {
            std::mem::transmute::<u8,Corner>(a & 0b111)
        }
    }
}
...
impl CornerMap {
    ...
    pub fn get(&self, corner: Corner) -> (Corner,Twist) {
        let m = (self.set >> (8 *corner as u64)) as u8;
        unsafe {
            (Corner::from(m),transmute(m>>3))
        }
    }
    pub fn from_iter(iter: impl Iterator<Item=(Corner,(Corner,Twist))>) -> Option<CornerMap> {
    ...
    pub fn inverse_multiply(self, b: CornerMap) -> CornerMap {
         CornerMap{set: map_inv_mul(self.set, b.set  )}
    }
    ...
    pub fn multiply(self, b: CornerMap) -> CornerMap {
         CornerMap{set: map_mul(self.set, b.set  )}
    }
    ...
    pub fn inverse(self) -> CornerMap {
        ...
        CornerMap{set: res ^ mnx ^(mnx<<1) }
    }

    pub fn print(self)  {
        for i in (0..8).rev() {
            eprint!("{:05b}_", (self.set >> ((7-i)*8))& 0xff);
        }
        eprintln!("");
        for i in (0..8).rev() {
            eprint!(" {:1}:{:1}  ", ((self.set >> ((7-i)*8))>>2)&0b11,(self.set >> ((7-i)*8))&0b111);
        }
        eprintln!("");
    }


    pub fn set_orientation_index(&mut self, mut index:u64)  {
        let mut sum = 0;
        let mut res = 0;
        let mut hp = |i:u64| {
            let m = index % 3;
        ...
        self.set &=  !0x00_18_18_18__18_18_18_18;
        self.set |= res;
        // self.fix_orientation();
    }

    pub fn orientation_index(self) -> u64 {
        let mut b = (self.set& 0x00_18_18_18__18_18_18_18)>>3;
        b *= 1+(1<<8)*3; // x3
    ...
    pub fn set_permutation_index(&mut self, index:u64) {
        ...
        self.set &= 0x18_18_18_18_18_18_18_18;
        self.set |= res;
    }
    pub fn permutation_index(self) -> u64 {
        const fn section(sh: u64) -> u64 {
            return 0b00111 << (sh*8);
        }
        let x:u64 = self.set & 0x0707070707070707;
...
impl std::convert::From<FaceMove> for CornerMap {
    ...
    fn from(turn: FaceMove) -> Self {
        let turns =&[
            CornerMap{set:0x0702050603000104},//Ucw
            CornerMap{set:0x0700050203040106},//U2
            CornerMap{set:0x0704050003060102},//Uccw
            CornerMap{set:0x0506010407020300},//Dcw
            CornerMap{set:0x0106030405020700},//D2
            CornerMap{set:0x0306070401020500},//Dccw
            CornerMap{set:0x07060c1003021509},//Fcw
            CornerMap{set:0x0706000103020405},//F2
            CornerMap{set:0x070609150302100c},//Fccw
            CornerMap{set:0x130f05040a160100},//Bcw
            CornerMap{set:0x0203050406070100},//B2
            CornerMap{set:0x160a05040f130100},//Bccw
            CornerMap{set:0x07060504110b0812},//Rcw
            CornerMap{set:0x0706050400010203},//R2
            CornerMap{set:0x0706050412080b11},//Rccw
            CornerMap{set:0x0e14170d03020100},//Lcw
            CornerMap{set:0x0405060703020100},//L2
            CornerMap{set:0x0d17140e03020100},//Lccw
        ];
        turns[turn as usize]
    }
}
...
mod tests {
    ...
    #[test]
    fn corner_index_mapping_A() {
        for pindex in 0..40320 {
            let mut cube = CornerMap::default();
            cube.set_permutation_index(pindex);
            for oindex in 0..2187 {
                let mut cube2 = cube;
                cube2.set_orientation_index(oindex);
                assert_eq!(oindex, cube2.orientation_index());
                assert_eq!(pindex, cube2.permutation_index());
                assert!(cube2.is_valid());
            }
        }
    }
    #[cfg(feature = "long-tests")]
    #[test]
    fn corner_index_mapping_B() {
        for oindex in 0..2187 {
            let mut cube = CornerMap::default();
            cube.set_orientation_index(oindex);
            for pindex in 0..40320 {
                let mut cube2 = cube;
                cube2.set_permutation_index(pindex);
                assert_eq!(oindex, cube2.orientation_index());
                assert_eq!(pindex, cube2.permutation_index());
                assert!(cube2.is_valid());
            }
